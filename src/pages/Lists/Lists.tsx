    ...
    return (
        <Layout>
            ...
            <ul>
                {data.lists.map(({id, title}, idx) => (
                    <Link  key={id} to={routes.lists.byId.absolute(id)}>
                        <li className={classNames('bg-gray-700 hover:bg-gray-800 rounded py-4 pr-3 text-white text-xs pl-4', {
                            'mt-1': idx !== 0
                        })}>
                        {title}
                        </li>
            ...
            <form className="mt-2" onSubmit={(e) => {
               ...
               <input type="text" className='bg-gray-700 rounded mt-2 py-4 pr-3 pl-4 text-white w-full' placeholder="Add task"/>
            </form>
        </Layout>
    )
}